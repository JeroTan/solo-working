...
name: planner
description: >-
  Use when user types /planner or asks for this novel workflow command. Step 4:
  Creates chapter structure, pacing, foreshadowing plan, and character arc
  mapping. Supports full-novel, per-arc, or batch planning (plan next N
  chapters).
---
# /planner

Treat text after `/planner` as `$ARGUMENTS`.
