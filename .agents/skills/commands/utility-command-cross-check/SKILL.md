...
description: >-
  Use when user types /utility-command-cross-check or asks for this novel
  workflow command. Cross-check a project file created by a workflow command
  against the current command instructions and suggest revisions when the file
  format is stale.
---
# /utility-command-cross-check

Treat text after `/utility-command-cross-check` as `$ARGUMENTS`.
